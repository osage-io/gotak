...
CREATE TABLE mfa_factors (
    ...
    -- Ensure unique active factors per type per user
    UNIQUE(user_id, type, status) 
...
CREATE TABLE mfa_auth_challenges (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
...
CREATE TABLE role_bindings (
    ...
    -- Ensure unique active role bindings
    UNIQUE(user_id, role_id, scope, scope_id, status)
        WHERE status = 'active'
);
...
CREATE TABLE encryption_keys (
    ...
    UNIQUE(key_id, status) WHERE status = 'active'
);
